...
3. **검사 통과 후 바로 배포** — 변경 내용을 보고하되, 사용자가 별도로 중단을 요청하지 않으면 배포 여부를 다시 묻지 않고 배포
...
- 게스트 종합의견·심사의견 작성 기능은 2026-09-07 구현되었다. 게스트는 각 이행항목 상세창의 07 종합의견·08 심사의견을 작성할 수 있다.
- 의견은 현재 `localStorage(shms.guest_opinions)`에만 저장되므로 같은 PC의 같은 브라우저에서만 유지된다. 다른 PC·브라우저 또는 다른 계정과 공동으로 보려면 Supabase 저장·인증 구조를 별도로 구현해야 한다.
- 일반 사용자 화면의 심사의견은 읽기 전용이지만, 같은 브라우저에 저장된 로컬 의견만 표시된다. 의견 내보내기 기능은 아직 없다.
