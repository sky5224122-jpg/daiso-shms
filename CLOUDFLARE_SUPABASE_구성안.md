...
- 사진(JPG·PNG·WebP)은 브라우저에서 WebP로 변환하며 1,280px부터 품질과 해상도를 단계적으로 낮춰 50KB를 목표로 저장한다. 최소 긴 변 560px·최저 품질 0.42를 보호선으로 유지하며, 목표를 넘더라도 이 품질 보호본은 저장한다.
- 사진·문서·ZIP 모두 압축 또는 변환 후 최대 15MB까지 저장한다. 15MB를 초과하는 PDF·Office·한글·ZIP은 앱에서 최고 압축 ZIP으로 자동 변환하며, 변환 후에도 15MB를 넘는 경우에만 Worker가 업로드를 차단한다.
