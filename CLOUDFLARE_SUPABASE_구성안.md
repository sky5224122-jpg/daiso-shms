...
- 사진(JPG·PNG·WebP)은 브라우저에서 WebP로 변환하며 1,280px부터 품질과 해상도를 단계적으로 낮춰 1장당 50KB 이하로 만든 뒤 저장. 저장 실패보다 기록 보존을 우선해 최소 96px·품질 0.08까지 자동 축소한다.
- 사진은 압축 후 최대 50KB, 10MB를 초과하는 PDF·Office·한글·ZIP은 앱에서 최고 압축 ZIP으로 자동 변환한다. 변환 후에도 10MB를 넘으면 Worker가 업로드를 차단한다.
